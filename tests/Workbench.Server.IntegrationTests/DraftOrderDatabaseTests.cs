...
public sealed partial class DraftOrderDatabaseTests(SqlServerFixture sqlServer)
{
    ...
    public async Task CurrencyTransitionRequiresAPriceClearingSaveAndMalformedPricesAreRejected()
    {
        ...
        // AND removing prices in the same request cannot skip the separately saved clearing step.
        Assert.Equal(50401, (await Assert.ThrowsAsync<SqlException>(() => Save(connection, actor, Guid.NewGuid(), Priced("Update", saved.Id, saved.Version, "EUR", null), "Update"))).Number);
        var cleared = await Save(connection, actor, Guid.NewGuid(), Priced("Update", saved.Id, saved.Version, "USD", null), "Update");
        cleared = await Save(connection, actor, Guid.NewGuid(), Priced("Update", saved.Id, cleared.Version, "EUR", null), "Update");
