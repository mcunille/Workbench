...
public sealed class SharedAcquisitionTests(SqlServerFixture sqlServer)
{
    ...
    public async Task ConnectReplaceRemovePreservesIdentityAndCreationReplay()
    {
        ...
        var created = await SendAsync(client, HttpMethod.Post, $"/api/beta/items/{first.Id}/acquisition", original);
        ...
        var connected = await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{second.Id}/acquisition-link", new
        {
        ...
        var page = await client.GetFromJsonAsync<JsonElement>($"/api/beta/acquisitions/{saved.Acquisition.Id}/items");
        Assert.Equal(2, page.GetProperty("items").GetArrayLength());
        var current = (await client.GetFromJsonAsync<ItemAcquisitionResponse>($"/api/beta/items/{first.Id}/acquisition"))!;
        // WHEN the original connection is removed and its old creation request replayed.
        Assert.Equal(HttpStatusCode.OK, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{first.Id}/acquisition-link", new
        {
        ...
        var replay = await SendAsync(client, HttpMethod.Post, $"/api/beta/items/{first.Id}/acquisition", original);
        ...
        var discoveries = await client.GetFromJsonAsync<JsonElement>("/api/beta/acquisitions?search=Family");
