...
public sealed class DraftOrderEndpointV4Tests(SqlServerFixture sqlServer)
{
    ...
    public async Task SupplierReplacementRequiresConfirmedChargeExplanation()
    {
        ...
        var draft = DraftOrderInputV4Tests.Empty with { SupplierName = "Supplier Alpha", Charges = [charge] };
        var created = await SendAsync(client, HttpMethod.Post, Path, new CreateDraftOrderRequestV4(Guid.NewGuid(), draft));
        Assert.Equal(HttpStatusCode.Created, created.StatusCode);
        var receipt = (await created.Content.ReadFromJsonAsync<SaveDraftOrderResponse>())!;
        var url = $"{Path}/{receipt.DraftOrderId}";
        var saved = (await client.GetFromJsonAsync<DraftOrderResponseV4>(url))!;
        var supplierResponse = await SendAsync(client, HttpMethod.Post, "/api/suppliers", new CreateSupplierRequest(Guid.NewGuid(), new("Supplier Alpha", null, null, null, null, null)));
        ...
        var identityRejected = await SendAsync(client, HttpMethod.Put, url, new UpdateDraftOrderRequestV4(Guid.NewGuid(), saved.Version, saved.Draft with { SupplierId = supplier.SupplierId }));
        ...
        {
            var rejected = await SendAsync(client, HttpMethod.Put, url, new UpdateDraftOrderRequestV4(Guid.NewGuid(), saved.Version, saved.Draft with { SupplierName = name }));
        ...
        var update = new UpdateDraftOrderRequestV4(Guid.NewGuid(), saved.Version, saved.Draft with { SupplierName = "Supplier Beta", Charges = [saved.Draft.Charges[0] with { Notes = "Original source; supplier replaced" }] });
