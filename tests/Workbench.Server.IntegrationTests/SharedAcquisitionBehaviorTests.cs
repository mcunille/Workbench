...
public sealed class SharedAcquisitionBehaviorTests(SqlServerFixture sqlServer)
{
    ...
    internal static async Task<ItemAcquisitionResponse> CreateContext(HttpClient client, ItemDetailResponse item, string source = "Fair") =>
        (await (await SendAsync(client, HttpMethod.Post, $"/api/beta/items/{item.Id}/acquisition",
    ...
    internal static async Task<ItemAcquisitionResponse> Current(HttpClient client, Guid id) =>
        (await client.GetFromJsonAsync<ItemAcquisitionResponse>($"/api/beta/items/{id}/acquisition"))!;
    ...
    public async Task ThreePiecesShareEditsAndArchiveRestoreRetainsRelationships()
    {
        ...
        {
            var result = await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{item.Id}/acquisition-link", Link(new(null, item.Version), saved.Acquisition));
        ...
        Assert.Equal(HttpStatusCode.OK, (await SendAsync(client, HttpMethod.Post, $"/api/beta/items/{items[2].Id}/archive", new { expectedVersion = archivedContext.ItemVersion })).StatusCode);
        var editContext = await Current(client, items[0].Id);
        var edited = await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{items[0].Id}/acquisition/{saved.Acquisition!.Id}",
        ...
        var path = $"/api/beta/acquisitions/{saved.Acquisition.Id}/items";
        ...
        Assert.Equal(HttpStatusCode.Conflict, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{archived.Id}/acquisition-link", Link(await Current(client, archived.Id), null))).StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await SendAsync(client, HttpMethod.Post, $"/api/beta/items/{archived.Id}/restore", new { expectedVersion = archived.Version })).StatusCode);
    ...
    public async Task AtomicReplacementLastRemovalAndCheckedNoOpPreserveSavedRecords()
    {
        ...
        Assert.Equal(HttpStatusCode.Conflict, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{first.Id}/acquisition-link", command with { TargetAcquisitionVersion = first.Version })).StatusCode);
        Assert.Equal(old, await Current(client, first.Id));
        // WHEN replacing atomically THEN the old empty context remains discoverable.
        Assert.Equal(HttpStatusCode.OK, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{first.Id}/acquisition-link", command)).StatusCode);
        Assert.Empty((await client.GetFromJsonAsync<AcquisitionItemsResponse>($"/api/beta/acquisitions/{old.Acquisition!.Id}/items"))!.Items);
        Assert.Single((await client.GetFromJsonAsync<AcquisitionPageResponse>("/api/beta/acquisitions?search=Old"))!.Items);
        Assert.Equal(HttpStatusCode.Conflict, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{first.Id}/acquisition-link", command)).StatusCode);
        var now = await Current(client, first.Id);
        var noOp = await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{first.Id}/acquisition-link", Link(now, now.Acquisition));
        ...
        Assert.Equal(HttpStatusCode.Conflict, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{first.Id}/acquisition-link", Link(now, now.Acquisition) with { ExpectedItemVersion = first.Version })).StatusCode);
        foreach (var item in new[] { first, second })
            Assert.Equal(HttpStatusCode.OK, (await SendAsync(client, HttpMethod.Put, $"/api/beta/items/{item.Id}/acquisition-link", Link(await Current(client, item.Id), null))).StatusCode);
        Assert.Empty((await client.GetFromJsonAsync<AcquisitionItemsResponse>($"/api/beta/acquisitions/{target.Acquisition!.Id}/items"))!.Items);
        var untouched = (await client.GetFromJsonAsync<ItemDetailResponse>($"/api/beta/items/{first.Id}"))!;
    ...
    public async Task DiscoveryIsBoundedStableSearchableAndTenantPrivate()
    {
        ...
        var response = await client.GetAsync("/api/beta/acquisitions?search=rare");
        ...
        var second = (await client.GetFromJsonAsync<AcquisitionPageResponse>("/api/beta/acquisitions?search=rare&cursor=" + Uri.EscapeDataString(first.NextCursor)))!;
        ...
        foreach (var query in new[] { "cursor=bad", "cursor=", "search=" + new string('x', 201) })
            Assert.Equal(HttpStatusCode.BadRequest, (await client.GetAsync("/api/beta/acquisitions?" + query)).StatusCode);
        ...
        {
            Assert.Equal(HttpStatusCode.NotFound, (await other.GetAsync($"/api/beta/acquisitions/{id}")).StatusCode);
            Assert.Equal(HttpStatusCode.NotFound, (await other.GetAsync($"/api/beta/acquisitions/{id}/items?includeArchived=true")).StatusCode);
        }
    }
}
