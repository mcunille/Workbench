...
{
    private static ReceiptDraftContentV1 Empty => new(null, null, null, null, [], []);
    private static ReceiptDraftEntryV1 Entry(string? price = null) => new(Guid.NewGuid(), null, null, null, price);
    ...
    public void EmptyAndNormalizedOptionalFactsPreserveUnknownAndExplicitZero()
    {
        ...
        var normalized = ReceiptDraftOrderInputV1.Normalize(input);
        // THEN blank optional fields remain unknown and zero retains an exact decimal meaning.
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(Empty));
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(normalized));
        Assert.Equal("Plan", normalized.Title);
        Assert.Equal("Supplier", normalized.SupplierName);
        Assert.Equal("USD", normalized.Currency);
        Assert.Null(ReceiptDraftOrderInputV1.Normalize(Empty with { Currency = " \t" }).Currency);
    ...
    public void NonExactPriceSyntaxIsRejected(string price)
    {
        ...
        Assert.Contains("draft.entries[0].indicativePrice", ReceiptDraftOrderInputV1.Validate(ReceiptDraftOrderInputV1.Normalize(input)).Keys);
    }
    ...
    public void CurrencyAndCollectionBoundsAreAuthoritative()
    {
        ...
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(Empty with { Currency = "USD", Entries = [Entry("999999999999999.9999")] }));
        // WHEN required price notation, identifiers and collection bounds are violated THEN field errors explain the limit.
        Assert.Contains("draft.currency", ReceiptDraftOrderInputV1.Validate(Empty with { Entries = [Entry("0")] }).Keys);
        foreach (var currency in new[] { "US", "USDD", "ÜSD", "U1D", " USD " })
            Assert.Contains("draft.currency", ReceiptDraftOrderInputV1.Validate(Empty with { Currency = currency }).Keys);
        var entry = Entry();
        Assert.Contains("draft.entries[1].id", ReceiptDraftOrderInputV1.Validate(Empty with { Entries = [entry, entry] }).Keys);
        Assert.Contains("draft.entries[0].id", ReceiptDraftOrderInputV1.Validate(Empty with { Entries = [entry with { Id = Guid.Empty }] }).Keys);
        Assert.Contains("draft.entries", ReceiptDraftOrderInputV1.Validate(Empty with { Entries = Enumerable.Range(0, 101).Select(_ => Entry()).ToArray() }).Keys);
        Assert.Contains("draft.sourceLinks", ReceiptDraftOrderInputV1.Validate(Empty with { SourceLinks = Enumerable.Repeat("https://example.com", 21).ToArray() }).Keys);
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(Empty with { SourceLinks = Enumerable.Repeat("https://example.com", 20).ToArray() }));
    }
    ...
    public void SafeAbsoluteLinksAreAcceptedWithoutFetchingOrRewriting(string link)
    {
        ...
        var input = ReceiptDraftOrderInputV1.Normalize(Empty with { SourceLinks = [link] });
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(input));
    ...
    public void UnsafeOrIncompleteLinksHaveFieldErrors(string link)
    {
        ...
        var errors = ReceiptDraftOrderInputV1.Validate(Empty with { SourceLinks = [link], Entries = [Entry() with { SourceLink = link }] });
    ...
    public void TextLimitsCountUtf16UnitsAndEscapedStorageBytes()
    {
        ...
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(input));
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(Empty with { Title = string.Concat(Enumerable.Repeat("\U0001F48E", 100)) }));
        // WHEN scalar or aggregate storage bounds are exceeded THEN no database write is necessary to reject them.
        Assert.Contains("draft.title", ReceiptDraftOrderInputV1.Validate(input with { Title = new string('t', 201) }).Keys);
        Assert.Contains("draft.title", ReceiptDraftOrderInputV1.Validate(Empty with { Title = string.Concat(Enumerable.Repeat("\U0001F48E", 101)) }).Keys);
        Assert.Contains("draft.supplierName", ReceiptDraftOrderInputV1.Validate(input with { SupplierName = new string('s', 201) }).Keys);
        Assert.Contains("draft.notes", ReceiptDraftOrderInputV1.Validate(input with { Notes = new string('n', 10001) }).Keys);
        Assert.Contains("draft", ReceiptDraftOrderInputV1.Validate(input with { Entries = input.Entries.Select(entry => entry with { Notes = new string('<', 2000) }).ToArray() }).Keys);
        Assert.Contains("draft.entries[0].description", ReceiptDraftOrderInputV1.Validate(Empty with { Entries = [Entry() with { Description = new string('d', 501) }] }).Keys);
        Assert.Contains("draft.entries[0].notes", ReceiptDraftOrderInputV1.Validate(Empty with { Entries = [Entry() with { Notes = new string('n', 2001) }] }).Keys);
        var longestLink = "https://example.com/" + new string('a', 2028);
        Assert.Equal(2048, longestLink.Length);
        Assert.Empty(ReceiptDraftOrderInputV1.Validate(Empty with { SourceLinks = [longestLink] }));
        Assert.Contains("draft.sourceLinks[0]", ReceiptDraftOrderInputV1.Validate(Empty with { SourceLinks = [longestLink + "x"] }).Keys);
    }
    ...
    public void CanonicalInputHasStableExplicitFieldOrderAndSemanticIdentity()
    {
        ...
        var canonical = ReceiptDraftOrderInputV1.Canonical("Create", null, null, ReceiptDraftOrderInputV1.Normalize(first));
        Assert.Equal(canonical, ReceiptDraftOrderInputV1.Canonical("Create", null, null, ReceiptDraftOrderInputV1.Normalize(second)));
        ...
        Assert.NotEqual(canonical, ReceiptDraftOrderInputV1.Canonical("Create", null, null, ReceiptDraftOrderInputV1.Normalize(second with { Notes = " " + "note" })));
        var ordered = second with { SourceLinks = ["https://one.example", "https://two.example"] };
        Assert.NotEqual(ReceiptDraftOrderInputV1.Canonical("Create", null, null, ordered), ReceiptDraftOrderInputV1.Canonical("Create", null, null, ordered with { SourceLinks = ordered.SourceLinks.Reverse().ToArray() }));
        Assert.Contains("\\u003C", ReceiptDraftOrderInputV1.Canonical("Create", null, null, Empty with { Notes = "<" }));
    }
    ...
    public void SavedVersionRequiresExactlyEightBase64Bytes()
    {
        ...
        Assert.Equal(expected, ReceiptDraftOrderInputV1.NormalizeVersion(expected, errors));
        Assert.Empty(errors);
        ...
        {
            errors.Clear();
            Assert.Null(ReceiptDraftOrderInputV1.NormalizeVersion(invalid, errors));
    ...
    public void WireContractRequiresExplicitNullsAndRejectsNestedUnknownFields()
    {
        ...
        Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<ReceiptDraftContentV1>("{\"sourceLinks\":[],\"entries\":[]}", options));
        Assert.Throws<JsonException>(() => JsonSerializer.Deserialize<ReceiptDraftContentV1>("{\"title\":null,\"supplierName\":null,\"currency\":null,\"notes\":null,\"sourceLinks\":[],\"entries\":[],\"tenantId\":null}", options));
    }
}
