...
export async function captureLayout(page, root) {
  await mkdir(root, { recursive: true });
  // Atomic output-directory creation reserves a bounded slot across worker processes.
  // The returned path is authoritative and contains no test-derived identifiers.
  for (let slot = 0; slot < 10; slot++) {
    const directory = join(root, `failure-${slot}`);
    ...
    let retained = false;
    try {
      const result = await captureReservedLayout(page, directory);
      retained = result !== undefined;
      return result;
    } finally {
...
async function captureReservedLayout(page, directory) {
