...
test('H5 cancellation and confirmed archive preserve a photographed bookmark across views and appearances', async ({ page }) => {
  ...
  const before = await (await page.request.get(`/api/beta/items/${item.id}`)).json();
  let submissions = 0;
  page.on('request', request => { if (request.url().endsWith(`/api/beta/items/${item.id}/archive`)) submissions++; });
  ...
  const after = await (await page.request.get(`/api/beta/items/${item.id}`)).json();
...
test('H5 lost committed response retries the same token and reviews saved archive state', async ({ page }) => {
  ...
  await page.route(`**/api/beta/items/${item.id}/archive`, async route => {
