...
export async function createArchived(page: Page, name = `H6 ${crypto.randomUUID()}`) {
  ...
  const created = await page.request.post('/api/beta/items', { headers: { 'X-Workbench-Api-Revision': 'beta-3', 'X-CSRF-TOKEN': csrf.requestToken }, data: { creationRequestId: crypto.randomUUID(), name, notes: 'Retained collection notes', location: 'Tray A' } });
...
export async function lifecycle(page: Page, id: string, action: 'archive' | 'restore', expectedVersion: string) {
  const csrf = await (await page.request.get('/api/beta/auth/antiforgery')).json();
  const response = await page.request.post(`/api/beta/items/${id}/${action}`, { headers: { 'X-Workbench-Api-Revision': 'beta-3', 'X-CSRF-TOKEN': csrf.requestToken }, data: { expectedVersion } });
