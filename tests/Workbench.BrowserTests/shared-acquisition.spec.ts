...
test('H10 a saved origin opens a shared acquisition without losing collection search', async ({ page }) => {
  ...
  const link = await page.request.put(`/api/beta/items/${sibling.id}/acquisition-link`, {
    headers: { 'X-Workbench-Api-Revision': 'beta-1', 'X-CSRF-TOKEN': csrf.requestToken },
...
test('H10 three stones share corrections, archive relationships and deliberate link removal', async ({ page }) => {
  ...
  const current = await (await page.request.get(`/api/beta/items/${stones[1].id}`)).json();
  ...
  expect((await page.request.get(`/api/beta/items/${stones[2].id}`)).status()).toBe(200);
  expect((await page.request.get(`/api/beta/acquisitions/${origin.acquisition.id}`)).status()).toBe(200);
});
...
test('H10 a lost connection response retains original tokens and requires review on retry', async ({ page }) => {
  ...
  await page.route(`**/api/beta/items/${second.id}/acquisition-link`, async route => {
...
test('H10 acquisition entry connects existing and newly saved pieces without duplicate item retries', async ({ page }) => {
  ...
  expect((await page.request.get(`/api/beta/acquisitions/${previous.acquisition.id}`)).status()).toBe(200);
  ...
  let creations = 0;
  page.on('request', request => { if (new URL(request.url()).pathname === '/api/beta/items' && request.method() === 'POST') creations++; });
  await page.getByRole('button', { name: 'Save item', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Review connection', exact: true })).toBeVisible();
  await page.route('**/api/beta/items/*/acquisition-link', route => route.fulfill({ status: 503 }), { times: 1 });
  await page.getByRole('button', { name: 'Save connection', exact: true }).click();
  ...
  const savedPieces = await (await page.request.get(`/api/beta/acquisitions/${target.acquisition.id}/items`)).json();
