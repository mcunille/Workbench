...
export async function verifyOwnerDataIsolation(live: Page, auth: Page) {
  ...
  const liveIdentity = await (await live.request.get('/api/auth/me')).json();
  const authIdentity = await (await auth.request.get('/api/auth/me')).json();
  ...
  const statuses = await Promise.all([
    live.request.get(`/api/items/${liveItem.id}`),
    live.request.get(`/api/items/${authItem.id}`),
    auth.request.get(`/api/items/${authItem.id}`),
    auth.request.get(`/api/items/${liveItem.id}`),
  ]);
  ...
  return async () => {
    ...
    expect((await live.request.get('/api/auth/me')).status()).toBe(200);
    expect((await live.request.get(`/api/items/${liveItem.id}`)).status()).toBe(200);
  };
}
