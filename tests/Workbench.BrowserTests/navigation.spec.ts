...
test('navigation respects the 900px collapse boundary and retains the mobile pill', async ({ page }) => {
  // GIVEN an authenticated desktop with animation disabled for exact geometry checks.
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.setViewportSize({ width: 900, height: 900 });
  await useAuthenticatedSession(page);
  const nav = page.getByRole('navigation', { name: 'Workspace' });
  // WHEN crossing each breakpoint THEN the expanded pane, rail, and pill fit the viewport.
  for (const width of [900, 899, 800, 768, 390, 320, 900]) {
    await page.setViewportSize({ width, height: 900 });
    if (width > 768) {
      await expect.poll(async () => (await nav.boundingBox())!.width).toBe(width >= 900 ? 229 : 72);
      if (width < 900) await expect(nav.getByRole('button', { name: 'Expand navigation' })).toBeVisible();
    } else {
      await expect.poll(async () => (await nav.boundingBox())!.y).toBeGreaterThan(700);
  ...
  // AND manual desktop collapse remains available after resizing back.
