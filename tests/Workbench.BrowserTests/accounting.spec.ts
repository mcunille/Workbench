...
test('accounting setup preserves explicit policies, guarded mappings, coverage, and role separation', async ({ page }) => {
  ...
  try {
    ...
    await page.getByLabel('State or region', { exact: true }).selectOption('WA');
    await page.getByLabel('Functional currency', { exact: true }).selectOption('USD');
    await expect(page.getByLabel('Posting decimal places', { exact: true })).toHaveValue('');
    await page.getByLabel('Posting decimal places', { exact: true }).selectOption('2');
    await page.getByLabel('Fiscal year starts', { exact: true }).selectOption('1');
    await page.getByLabel('Starting approach', { exact: true }).selectOption('FromBeginning');
    ...
    await page.reload();
    await expect(page.getByLabel('Functional currency', { exact: true })).toHaveValue('USD');
    await expect(page.getByLabel('Posting decimal places', { exact: true })).toHaveValue('2');
    ...
    await page.getByLabel('Supplier Payable', { exact: true }).selectOption({ label: '2000 — Supplier payables' });
    await page.getByRole('button', { name: 'Save setup', exact: true }).click();
    await expect(page.getByText('Accounting setup saved. Bookkeeping is not yet available.')).toBeVisible();
    ...
    await expect(page.getByLabel('Account purpose', { exact: true })).toBeDisabled();
    ...
    await bank.getByLabel('Accounting perimeter', { exact: true }).selectOption('include');
    await bank.getByLabel('Evidence basis', { exact: true }).selectOption('NoPriorActivity');
