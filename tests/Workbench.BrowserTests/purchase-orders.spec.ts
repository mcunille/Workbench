...
async function save(page: Page) {
  const response = page.waitForResponse(r => r.url().includes('/api/beta/purchase-order-drafts') && !r.url().endsWith('/calculate') &&
...
test('uncertain creation retries the original request without creating another draft', async ({ page }) => {
  ...
  const response = await page.request.get('/api/beta/purchase-order-drafts');
