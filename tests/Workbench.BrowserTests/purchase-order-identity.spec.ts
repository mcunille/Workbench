...
async function saveDraft(page: Page) {
  const saved = page.waitForResponse(response => response.url().includes('/api/v3/purchase-order-drafts') && !response.url().endsWith('/calculate') &&
  ...
  const detail = await page.request.get(`/api/v3/purchase-order-drafts/${page.url().split('/').at(-1)}`);
...
test('one supplier has independent order snapshots and platforms with deliberate contact refresh', async ({ page }) => {
  ...
  await page.getByRole('button', { name: `Select ${supplierName}`, exact: true }).click();
