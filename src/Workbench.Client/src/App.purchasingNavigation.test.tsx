...
beforeEach(() => {
  vi.spyOn(purchasingApi, 'calculateDraft').mockImplementation(() => new Promise(() => {}));
  ...
  vi.spyOn(purchasingApi, 'getDrafts').mockResolvedValue({ items: [first, second].map(value => ({ id: value.id, title: value.draft.title, supplierName: null, firstItemDescription: null, poReference: value.poReference, supplierOrderReference: null, platform: null, updatedAtUtc: value.updatedAtUtc })), nextCursor: null });
...
it('keeps unsaved input and navigation protection when history jumps to another entry for the same draft', async () => {
  ...
  await waitFor(() => expect(screen.getByLabelText('Custom title (optional)')).toHaveValue('First draft'));
...
it('loads the different draft when history jumps directly between editor routes', async () => {
  ...
  await act(async () => { render(<App />); });
  await waitFor(() => expect(screen.getByLabelText('Custom title (optional)')).toHaveValue('Second draft'));
  await click(screen.getByRole('button', { name: 'Back to purchase orders' }));
  await click(await screen.findByRole('link', { name: /First draft/ }));
  await waitFor(() => expect(screen.getByLabelText('Custom title (optional)')).toHaveValue('First draft'));
  ...
  expect(screen.getByLabelText('Custom title (optional)')).toHaveValue('Second draft');
