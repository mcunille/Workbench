...
it('opens purchase orders and replaces a new draft URL after its receipt without losing current-read recovery', async () => {
  ...
  server.use(http.get('*/api/beta/system', () => HttpResponse.json({ name: 'Workbench', version: '1', apiRevision: 'beta-3' })),
    ...
    http.post('*/api/beta/purchase-order-drafts/calculate', () => HttpResponse.json(zeroAdjustmentCalculation({ lines: [], incompleteLineCount: 0, merchandiseEstimate: null }))),
    http.post('*/api/beta/purchase-order-drafts', () => { writes++; return HttpResponse.json({ requestId: 'request', replayed: false, draftOrderId: 'saved', savedVersion: 'v1', completedAtUtc: '2026-09-12T00:00:00Z' }, { status: 201 }); }),
    http.get('*/api/beta/purchase-orders/saved', () => ++reads === 1 ? new HttpResponse(null, { status: 503 }) : HttpResponse.json({ calculation: zeroAdjustmentCalculation({ lines: [], incompleteLineCount: 0, merchandiseEstimate: null }), id: 'saved', poReference: 'PO-000001', supplierIsArchived: false, draft, version: 'v1', createdAtUtc: '2026-09-12T00:00:00Z', updatedAtUtc: '2026-09-12T00:00:00Z' })));
  render(<App />);
