...
it('opens purchase orders and replaces a new draft URL after its receipt without losing current-read recovery', async () => {
  ...
  server.use(http.get('*/api/system', () => HttpResponse.json({ name: 'Workbench', version: '1' })),
    ...
    http.get('*/api/v3/purchase-order-drafts', () => HttpResponse.json({ items: [], nextCursor: null })),
    http.get('*/api/auth/antiforgery', () => HttpResponse.json({ requestToken: 'test' })),
    http.post('*/api/v3/purchase-order-drafts/calculate', () => HttpResponse.json({ lines: [], incompleteLineCount: 0, merchandiseEstimate: null })),
    http.post('*/api/v3/purchase-order-drafts', () => { writes++; return HttpResponse.json({ requestId: 'request', replayed: false, draftOrderId: 'saved', savedVersion: 'v1', completedAtUtc: '2026-09-12T00:00:00Z' }, { status: 201 }); }),
    http.get('*/api/v3/purchase-order-drafts/saved', () => ++reads === 1 ? new HttpResponse(null, { status: 503 }) : HttpResponse.json({ calculation: { lines: [], incompleteLineCount: 0, merchandiseEstimate: null }, id: 'saved', poReference: 'PO-000001', supplierIsArchived: false, draft, version: 'v1', createdAtUtc: '2026-09-12T00:00:00Z', updatedAtUtc: '2026-09-12T00:00:00Z' })));
  render(<App />);
