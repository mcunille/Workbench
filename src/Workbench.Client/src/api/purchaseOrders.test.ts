...
it('sends exact decimal strings and a protected full replacement, returning a compact receipt', async () => {
  ...
  server.use(http.get('*/api/auth/antiforgery', () => HttpResponse.json({ requestToken: 'csrf-test' })), http.put('*/api/v4/purchase-order-drafts/draft', async ({ request }) => { received = await request.json(); csrf = request.headers.get('X-CSRF-TOKEN'); return HttpResponse.json(receipt); }));
...
it('encodes opaque cursors and makes private uncached detail and page reads', async () => {
  ...
  server.use(http.get('*/api/v4/purchase-order-drafts', ({ request }) => { requests.push(request); return HttpResponse.json({ items: [], nextCursor: null }); }), http.get('*/api/v4/purchase-order-drafts/draft', ({ request }) => { requests.push(request); return HttpResponse.json({ id: 'draft', draft }); }));
...
it('preserves validation paths and distinguishes request collisions from stale versions', async () => {
  ...
  server.use(http.get('*/api/auth/antiforgery', () => HttpResponse.json({ requestToken: 'csrf-test' })), http.post('*/api/v4/purchase-order-drafts', () => HttpResponse.json({ code: 'draft_validation_failed', errors: { 'draft.currency': ['Required with a price.'] } }, { status: 400 })), http.put('*/api/v4/purchase-order-drafts/draft', () => HttpResponse.json({ code: 'draft_request_conflict' }, { status: 409 })));
...
it('sends protected deletion with an exact request and version and returns its receipt', async () => {
  ...
  let received: unknown; let csrf: string | null = null;
  server.use(http.get('*/api/auth/antiforgery', () => HttpResponse.json({ requestToken: 'csrf-test' })), http.delete('*/api/v4/purchase-order-drafts/draft', async ({ request }) => {
