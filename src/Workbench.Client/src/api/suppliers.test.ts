...
it('sends private query-bound reads and exact protected supplier commands', async () => {
  ...
  server.use(http.get('*/api/beta/auth/antiforgery', () => HttpResponse.json({ requestToken: 'supplier-csrf' })),
    http.get('*/api/beta/suppliers', ({ request }) => { reads.push(request); return HttpResponse.json({ items: [record], nextCursor: null }); }),
    http.get('*/api/beta/suppliers/one', ({ request }) => { reads.push(request); return HttpResponse.json(record); }),
    http.post('*/api/beta/suppliers', handle), http.put('*/api/beta/suppliers/one', handle), http.post('*/api/beta/suppliers/one/archive', handle));
...
it('retains stable supplier field errors and concurrency codes', async () => {
  ...
  server.use(http.get('*/api/beta/auth/antiforgery', () => HttpResponse.json({ requestToken: 'csrf' })), http.post('*/api/beta/suppliers', () => HttpResponse.json({ code: 'supplier_validation_failed', errors: { 'supplier.email': ['Invalid email.'], unexpected: 4 } }, { status: 400 })), http.post('*/api/beta/suppliers/one/archive', () => HttpResponse.json({ code: 'supplier_version_conflict' }, { status: 409 })));
