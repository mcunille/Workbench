...
it('sends multipart bytes with CSRF and immutable command evidence', async () => {
  ...
  await uploadPurchaseDocument('order', { ...command, file: new File(['original'], 'receipt.pdf', { type: 'text/plain' }) });
  expect(captured?.get('requestId')).toBe('request');
  expect(captured?.get('expectedOrderVersion')).toBe('v1');
...
it('does not initiate a browser download when authenticated delivery fails', async () => {
  ...
  await expect(downloadPurchaseDocument('order', { id: 'doc', label: 'Private label', mediaType: 'application/pdf', extension: '.pdf', length: 1, createdAtUtc: '', version: 'd1', unavailable: false })).rejects.toBeInstanceOf(ApiError);
...
it('preserves the server format rejection title for actionable correction', async () => {
  ...
  await expect(uploadPurchaseDocument('order', { ...command, file: new File(['x'], 'x.pdf') })).rejects.toMatchObject({ errors: { document: ['Encrypted PDF documents are not supported.'] } });
});
