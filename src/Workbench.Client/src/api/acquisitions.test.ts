...
it('encodes discovery and archived pagination and requests private uncached reads', async () => {
  ...
  server.use(http.get('*/api/beta/acquisitions', ({ request }) => { requests.push(request); return HttpResponse.json({ items: [], nextCursor: null }); }),
    http.get('*/api/beta/acquisitions/fair/items', ({ request }) => { requests.push(request); return HttpResponse.json({ items: [], nextCursor: null }); }));
...
it('sends one protected conditional link command and surfaces stale state as a conflict', async () => {
  ...
  server.use(http.get('*/api/beta/auth/antiforgery', () => HttpResponse.json({ requestToken: 'csrf-test' })),
    http.put('*/api/beta/items/stone/acquisition-link', async ({ request }) => {
