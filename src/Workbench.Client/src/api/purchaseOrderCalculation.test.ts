...
it('previews the exact structured draft without a save request and protects the request', async () => {
  ...
  const draft: DraftContent = { title: null, supplierName: null, supplierId: null, supplierContactName: null, supplierEmail: null, supplierPhone: null, supplierWebsite: null, supplierPostalAddress: null, supplierOrderReference: null, platform: null, currency: 'USD', notes: null, sourceLinks: [], entries: [{ ...emptyLine('line'), quantity: '12.5000', unitOfMeasure: 'carat', unitPrice: '0.0001', pricingUnit: 'carat', pricePerQuantity: '1' }] };
  let requestBody: unknown; let token: string | null = null; let cache: RequestCache | undefined;
  const response = { lines: [{ id: 'line', gross: '0.0013' }], incompleteLineCount: 0, merchandiseEstimate: '0.0013' };
  server.use(http.get('*/api/auth/antiforgery', () => HttpResponse.json({ requestToken: 'preview-csrf' })), http.post('*/api/v3/purchase-order-drafts/calculate', async ({ request }) => { requestBody = await request.json(); token = request.headers.get('X-CSRF-TOKEN'); cache = request.cache; return HttpResponse.json(response); }));
