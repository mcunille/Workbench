...
it('previews the exact structured draft without a save request and protects the request', async () => {
  ...
  server.use(http.get('*/api/auth/antiforgery', () => HttpResponse.json({ requestToken: 'preview-csrf' })), http.post('*/api/v4/purchase-order-drafts/calculate', async ({ request }) => { requestBody = await request.json(); token = request.headers.get('X-CSRF-TOKEN'); cache = request.cache; return HttpResponse.json(response); }));
