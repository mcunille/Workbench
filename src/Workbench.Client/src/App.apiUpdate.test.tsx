...
it('keeps the draft and uncertain save in place when the deployed API changes', async () => {
  ...
  server.use(
    http.get('*/api/beta/system', () => HttpResponse.json({ name: 'Workbench', version: 'beta', apiRevision: 'beta-3' })),
    ...
    http.post('*/api/beta/purchase-order-drafts', async ({ request }) => {
      expect(request.headers.get('X-Workbench-Api-Revision')).toBe('beta-3');
