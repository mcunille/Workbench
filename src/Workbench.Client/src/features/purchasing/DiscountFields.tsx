...
export function DiscountFields({ label, path, discount, base, amount, currency, disabled, errors, change }: {
  ...
  return <fieldset className="po-discount" disabled={disabled} id={financialFieldId(path)}>
    ...
    <div className="po-adjustment-fields">
      <div className="po-field"><FloatingField htmlFor={modeId} label="Discount type" compact><select id={modeId} aria-label={`${label} type`} value={discount.mode} aria-invalid={!!errors[`${path}.mode`]} onChange={event => change({ mode: event.target.value, value: '' })}><option value="fixed">Fixed amount</option><option value="percentage">Percentage</option></select></FloatingField>{errors[`${path}.mode`] ? <p className="form-message error">{errors[`${path}.mode`].join(' ')}</p> : null}</div>
    ...
    <p className="po-field-help">{base === undefined ? 'Calculating discount…' : base === null ? 'Eligible base: Unknown. Enter the missing line prices or quantities.' : `Eligible base: ${currency} ${formatReferencePrice(base)}${amount == null ? '' : ` · Reduction: ${currency} ${formatReferencePrice(amount)}`}`}{label === 'Order discount' ? ' Applies to merchandise after line discounts, excluding all charges.' : ' Applies to this line’s gross amount.'}</p>
    <button type="button" className="quiet" onClick={() => { change(null); requestAnimationFrame(() => button.current?.focus()); }}>Remove {label.startsWith('Line') ? 'line discount' : 'order discount'}</button>
  </fieldset>;
}
