...
it('preserves existing extra precision and exact large amounts', () => {
  ...
  expect(toggle).toBeEnabled(); fireEvent.click(toggle); expect(toggle).not.toBeChecked();
});
...
it('deletes selected digits rather than the last digit', () => {
  // GIVEN a price whose whole-unit digits are selected.
  render(<Harness initial="12.34" />);
  const field = () => screen.getByLabelText('Reference price 1') as HTMLInputElement;
  field().focus(); field().setSelectionRange(0, 2);
  // WHEN deleting the selection THEN the unselected cents are retained.
