...
export function RecoveryText({ label, text }: { label: string; text: string }) {
  ...
  return <section className="recovery-text" aria-label={`${label} recovery`}>
    ...
    <div><button type="button" className="secondary" onClick={() => { field.current?.focus(); field.current?.select(); }}>Select {label.toLowerCase()} text</button></div>
  </section>;
}
