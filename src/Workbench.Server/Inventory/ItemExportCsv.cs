...
public sealed record ExportItem(Guid Id, string TrackingKind, string Name, string? Notes,
    string? Location, DateTimeOffset CreatedAtUtc, DateTimeOffset? ArchivedAtUtc);
...
public static class ItemExportCsv
{
    ...
    public const string Header = "schema_version,exported_at_utc,scope,item_id,tracking_kind,name,notes,location,is_archived,created_at_utc,archived_at_utc";
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var item in items)
        {
            ...
            WriteRow(["1", Timestamp(exportedAt), scope, item.Id.ToString("D"), item.TrackingKind,
