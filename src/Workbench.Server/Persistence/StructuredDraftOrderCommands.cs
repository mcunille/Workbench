...
{
    private const string SaveDraftV3 = """
        CREATE PROCEDURE [Purchasing].[SaveDraftOrderV3]
            ...
            BEGIN TRY
                ...
                BEGIN
                    IF NOT EXISTS(SELECT 1 FROM Purchasing.DraftOrderRequestReceipts WHERE TenantId=@TenantId AND RequestId=@RequestId
                        AND Operation=@Operation AND FingerprintVersion=3 AND InputFingerprint=@Fingerprint
                ...
                IF EXISTS(SELECT 1 FROM OPENJSON(@Entries) e WHERE (SELECT COUNT(*) FROM @EntryFields p WHERE p.EntryIndex=CONVERT(int,e.[key]))<>13)
                    ...
                    OR EXISTS(SELECT 1 FROM @EntryFields WHERE CONVERT(varbinary(max),[key]) NOT IN(
                        ...
                        CONVERT(varbinary(max),N'sourceLink'),CONVERT(varbinary(max),N'indicativePrice'),CONVERT(varbinary(max),N'quantity'),CONVERT(varbinary(max),N'unitOfMeasure'),CONVERT(varbinary(max),N'unitPrice'),CONVERT(varbinary(max),N'pricingUnit'),CONVERT(varbinary(max),N'pricePerQuantity'),CONVERT(varbinary(max),N'pricingQuantity'),CONVERT(varbinary(max),N'supplierSku'),CONVERT(varbinary(max),N'itemType')) OR [type] NOT IN(0,1))
                ...
                IF EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'indicativePrice',N'unitPrice') AND [type]<>0 AND
                    ...
                    THROW 50400,'Review the draft fields.',1;
                IF EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'quantity',N'pricePerQuantity',N'pricingQuantity') AND [type]<>0 AND
                    ...
                    OR EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'unitOfMeasure',N'pricingUnit') AND [type]<>0 AND
                ...
                DECLARE @Lines TABLE(Quantity decimal(13,4),Unit nvarchar(20),Price decimal(19,4),PricingUnit nvarchar(20),Denominator decimal(13,4),PricingQuantity decimal(13,4),Reference decimal(19,4));
                INSERT @Lines SELECT TRY_CONVERT(decimal(13,4),JSON_VALUE([value],'$.quantity')),JSON_VALUE([value],'$.unitOfMeasure'),TRY_CONVERT(decimal(19,4),JSON_VALUE([value],'$.unitPrice')),JSON_VALUE([value],'$.pricingUnit'),TRY_CONVERT(decimal(13,4),JSON_VALUE([value],'$.pricePerQuantity')),TRY_CONVERT(decimal(13,4),JSON_VALUE([value],'$.pricingQuantity')),TRY_CONVERT(decimal(19,4),JSON_VALUE([value],'$.indicativePrice')) FROM OPENJSON(@Entries);
                IF EXISTS(SELECT 1 FROM @Lines WHERE (Price IS NOT NULL AND Reference IS NOT NULL) OR (PricingQuantity IS NOT NULL AND (Unit IS NULL OR PricingUnit IS NULL OR Unit=PricingUnit)))
                    THROW 50400,'Review line quantity and pricing fields.',1;
                -- Compare exact scaled integers before division. Rounded gross may contain at most 23 scaled digits.
                -- N / D rounds past the maximum exactly when 2*N >= (2*10^23-1)*D.
                ...
                BEGIN
                    ...
                    IF @CurrentCurrency IS NOT NULL AND (@Currency IS NULL OR CONVERT(varbinary(max),CONVERT(nvarchar(3),@CurrentCurrency))<>CONVERT(varbinary(max),@Currency))
                        AND (EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'indicativePrice',N'unitPrice') AND [type]<>0)
                            OR EXISTS(SELECT 1 FROM Purchasing.DraftOrders d CROSS APPLY OPENJSON(d.ContentJson,'$.entries') e
                                WHERE d.TenantId=@TenantId AND d.Id=@TargetId
                                  AND (JSON_VALUE(e.[value],'$.indicativePrice') IS NOT NULL OR JSON_VALUE(e.[value],'$.unitPrice') IS NOT NULL)))
                    ...
                    UPDATE Purchasing.DraftOrders SET Title=@Title,SupplierName=@Supplier,Currency=@Currency,Notes=@Notes,
                        ...
                        ContentSchemaVersion=2,ContentJson=@Content,UpdatedAtUtc=@Now,UpdatedByUserId=@ActorUserId
                ...
                BEGIN
                    ...
                    INSERT Purchasing.DraftOrders(PoNumber,SupplierId,SupplierContactName,SupplierEmail,SupplierPhone,SupplierWebsite,SupplierPostalAddress,SupplierOrderReference,Platform,Id,TenantId,Title,SupplierName,Currency,Notes,ContentSchemaVersion,ContentJson,CreatedAtUtc,UpdatedAtUtc,CreatedByUserId,UpdatedByUserId)
                        VALUES(@PoNumber,@SupplierId,@ContactName,@Email,@Phone,@Website,@PostalAddress,@SupplierOrderReference,@Platform,@TargetId,@TenantId,@Title,@Supplier,@Currency,@Notes,2,@Content,@Now,@Now,@ActorUserId,@ActorUserId);
                END;
                ...
                INSERT Purchasing.DraftOrderRequestReceipts(TenantId,RequestId,DraftOrderId,Operation,ActorUserId,ExpectedRowVersion,FingerprintVersion,InputFingerprint,ResultRowVersion,CompletedAtUtc)
                    VALUES(@TenantId,@RequestId,@TargetId,@Operation,@ActorUserId,@ExpectedVersion,3,@Fingerprint,@CurrentVersion,@Now);
                COMMIT;
