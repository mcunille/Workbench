...
internal static class DraftOrderSchema
{
    internal static void Protect(MigrationBuilder migrationBuilder, string migrationId)
    {
        foreach (var table in new[] { "DraftOrders", "DraftOrderRequestReceipts" })
            migrationBuilder.Sql($"""
                ...
                """);
        migrationBuilder.Sql(SourceLinkValidation);
        migrationBuilder.Sql(Save);
        foreach (var operation in new[] { "Create", "Update" })
        {
            migrationBuilder.Sql($"""
                CREATE PROCEDURE [Purchasing].[{operation}DraftOrder]
                    @RequestId uniqueidentifier,@CanonicalInputJson nvarchar(max),@ActorUserId uniqueidentifier
                AS
                BEGIN
                    SET NOCOUNT ON;
        ...
        migrationBuilder.Sql($"""
            ...
            SET @Readiness=REPLACE(@Readiness,N'20260911184933_AddAcquisitionDocuments',N'{migrationId}');
            EXEC sys.sp_executesql @Readiness;
            """);
    }

    private const string SourceLinkValidation = """
        CREATE FUNCTION [Purchasing].[IsDraftSourceLink](@Value nvarchar(max)) RETURNS bit AS
        BEGIN
            IF @Value IS NULL RETURN 1;
            IF DATALENGTH(@Value) NOT BETWEEN 2 AND 4096 OR CHARINDEX(N'\',@Value)>0 RETURN 0;
            DECLARE @SchemeEnd int=CHARINDEX(N'://',@Value);
            IF @SchemeEnd NOT IN (5,6) RETURN 0;
            IF LOWER(LEFT(@Value,@SchemeEnd-1)) COLLATE Latin1_General_100_BIN2 NOT IN (N'http',N'https') RETURN 0;
            DECLARE @Rest nvarchar(max)=SUBSTRING(@Value,@SchemeEnd+3,2048),@End int,@Authority nvarchar(2048),@Host nvarchar(2048),@Port nvarchar(20);
            SET @End=PATINDEX(N'%[/#?]%',@Rest);
            SET @Authority=CASE WHEN @End=0 THEN @Rest ELSE LEFT(@Rest,@End-1) END;
            IF DATALENGTH(@Authority)=0 OR CHARINDEX(N'@',@Authority)>0 RETURN 0;
            -- Reject controls/whitespace and URI delimiters that cannot be part of a hostname.
            IF @Authority COLLATE Latin1_General_100_BIN2 LIKE N'%[ <>"\|]%' RETURN 0;
            ...
            BEGIN
                ...
                IF @End<4 RETURN 0;
                SET @Host=SUBSTRING(@Authority,2,@End-2);
                IF CHARINDEX(N':',@Host)=0 OR @Host COLLATE Latin1_General_100_BIN2 LIKE N'%[^0-9A-Fa-f:.]%' RETURN 0;
                -- An embedded IPv4 suffix represents two IPv6 groups. Validate it before counting groups.
                DECLARE @Ipv6 nvarchar(2048)=@Host,@Ipv4 nvarchar(2048),@LastColon int,@Compression int,@Groups int;
                IF CHARINDEX(N'.',@Ipv6)>0
                BEGIN
                    SET @LastColon=LEN(@Ipv6)-CHARINDEX(N':',REVERSE(@Ipv6))+1;
                    SET @Ipv4=SUBSTRING(@Ipv6,@LastColon+1,2048);
                    IF (SELECT COUNT(*) FROM STRING_SPLIT(@Ipv4,N'.'))<>4
                        OR EXISTS(SELECT 1 FROM STRING_SPLIT(@Ipv4,N'.') WHERE DATALENGTH([value]) NOT BETWEEN 2 AND 6
                            OR [value] COLLATE Latin1_General_100_BIN2 LIKE N'%[^0-9]%'
                            OR TRY_CONVERT(int,[value]) NOT BETWEEN 0 AND 255) RETURN 0;
                    SET @Ipv6=LEFT(@Ipv6,@LastColon)+N'0:0';
                END;
                SET @Compression=CHARINDEX(N'::',@Ipv6);
                IF (@Compression>0 AND CHARINDEX(N'::',@Ipv6,@Compression+1)>0)
                    OR (LEFT(@Ipv6,1)=N':' AND LEFT(@Ipv6,2)<>N'::')
                    OR (RIGHT(@Ipv6,1)=N':' AND RIGHT(@Ipv6,2)<>N'::') RETURN 0;
                IF EXISTS(SELECT 1 FROM STRING_SPLIT(@Ipv6,N':') WHERE DATALENGTH([value])>8
                    OR [value] COLLATE Latin1_General_100_BIN2 LIKE N'%[^0-9A-Fa-f]%') RETURN 0;
                SELECT @Groups=COUNT(*) FROM STRING_SPLIT(@Ipv6,N':') WHERE DATALENGTH([value])>0;
                IF (@Compression=0 AND @Groups<>8) OR (@Compression>0 AND @Groups>=8) RETURN 0;
                IF DATALENGTH(@Authority)>@End*2
                BEGIN
                    IF SUBSTRING(@Authority,@End+1,1)<>N':' RETURN 0;
                    SET @Port=SUBSTRING(@Authority,@End+2,20);
                END;
            END
            ELSE
            BEGIN
            ...
            BEGIN TRY
                ...
                BEGIN
                    IF NOT EXISTS(SELECT 1 FROM Purchasing.DraftOrderRequestReceipts WHERE TenantId=@TenantId AND RequestId=@RequestId
                        AND Operation=@Operation AND FingerprintVersion=1 AND InputFingerprint=@Fingerprint
