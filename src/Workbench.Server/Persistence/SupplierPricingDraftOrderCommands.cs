...
{
    private const string SaveDraftV4 = """
        CREATE PROCEDURE [Purchasing].[SaveDraftOrderV4]
            ...
            BEGIN TRY
                ...
                BEGIN
                    IF NOT EXISTS(SELECT 1 FROM Purchasing.DraftOrderRequestReceipts WHERE TenantId=@TenantId AND RequestId=@RequestId
                        AND Operation=@Operation AND FingerprintVersion=4 AND InputFingerprint=@Fingerprint
                ...
                IF EXISTS(SELECT 1 FROM OPENJSON(@Entries) e WHERE (SELECT COUNT(*) FROM @EntryFields p WHERE p.EntryIndex=CONVERT(int,e.[key]))<>12)
                    ...
                    OR EXISTS(SELECT 1 FROM @EntryFields WHERE CONVERT(varbinary(max),[key]) NOT IN(
                        ...
                        CONVERT(varbinary(max),N'sourceLink'),CONVERT(varbinary(max),N'indicativePrice'),CONVERT(varbinary(max),N'quantity'),CONVERT(varbinary(max),N'unitOfMeasure'),CONVERT(varbinary(max),N'price'),CONVERT(varbinary(max),N'priceMode'),CONVERT(varbinary(max),N'legacyPricing'),CONVERT(varbinary(max),N'supplierSku'),CONVERT(varbinary(max),N'itemType')) OR ([key]<>N'legacyPricing' AND [type] NOT IN(0,1)) OR ([key]=N'legacyPricing' AND [type] NOT IN(0,5)))
                ...
                IF EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'indicativePrice',N'price') AND [type]<>0 AND
                    ...
                    THROW 50400,'Review the draft fields.',1;
                IF EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'quantity') AND [type]<>0 AND
                    (DATALENGTH([value]) NOT BETWEEN 12 AND 28 OR [value] COLLATE Latin1_General_100_BIN2 LIKE N'%[^0-9.]%'
                    OR CHARINDEX(N'.',[value])<>DATALENGTH([value])/2-4 OR CHARINDEX(N'.',[value],CHARINDEX(N'.',[value])+1)>0
                    OR (LEFT([value],1)=N'0' AND CHARINDEX(N'.',[value])<>2) OR TRY_CONVERT(decimal(13,4),[value]) IS NULL OR TRY_CONVERT(decimal(13,4),[value])<=0))
                    OR EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'unitOfMeasure') AND [type]<>0 AND
                       CONVERT(varbinary(max),[value]) NOT IN(CONVERT(varbinary(max),N'piece'),CONVERT(varbinary(max),N'carat'),CONVERT(varbinary(max),N'gram'),CONVERT(varbinary(max),N'kilogram'),CONVERT(varbinary(max),N'ounce'),CONVERT(varbinary(max),N'troyOunce'),CONVERT(varbinary(max),N'millimeter'),CONVERT(varbinary(max),N'centimeter'),CONVERT(varbinary(max),N'meter'),CONVERT(varbinary(max),N'parcel'),CONVERT(varbinary(max),N'pair'),CONVERT(varbinary(max),N'set'),CONVERT(varbinary(max),N'pack'),CONVERT(varbinary(max),N'box'),CONVERT(varbinary(max),N'lot')))
                    OR EXISTS(SELECT 1 FROM @EntryFields WHERE ([key]=N'supplierSku' AND DATALENGTH([value])>400) OR ([key]=N'itemType' AND DATALENGTH([value])>200) OR ([key] IN(N'supplierSku',N'itemType') AND [type]<>0 AND (DATALENGTH(TRIM(@Whitespace FROM [value]))=0 OR CONVERT(varbinary(max),[value])<>CONVERT(varbinary(max),TRIM(@Whitespace FROM [value])))))
                    THROW 50400,'Review line quantity and pricing fields.',1;
                IF EXISTS(SELECT 1 FROM @EntryFields WHERE [key]=N'priceMode' AND ([type]<>1 OR CONVERT(varbinary(max),[value]) NOT IN(CONVERT(varbinary(max),N'perUnit'),CONVERT(varbinary(max),N'lineTotal'))))
                    THROW 50400,'Choose a pricing mode.',1;
                DECLARE @Lines TABLE(Quantity decimal(13,4),Unit nvarchar(20),Price decimal(23,4),Mode nvarchar(20),Reference decimal(19,4),Legacy nvarchar(max));
                INSERT @Lines SELECT TRY_CONVERT(decimal(13,4),JSON_VALUE([value],'$.quantity')),JSON_VALUE([value],'$.unitOfMeasure'),TRY_CONVERT(decimal(23,4),JSON_VALUE([value],'$.price')),JSON_VALUE([value],'$.priceMode'),TRY_CONVERT(decimal(19,4),JSON_VALUE([value],'$.indicativePrice')),JSON_QUERY([value],'$.legacyPricing') FROM OPENJSON(@Entries);
                IF EXISTS(SELECT 1 FROM @Lines WHERE (Price IS NOT NULL AND (Reference IS NOT NULL OR Legacy IS NOT NULL)) OR (Mode=N'perUnit' AND Price>=1000000000000000))
                    OR EXISTS(SELECT 1 FROM @EntryFields WHERE [key]=N'indicativePrice' AND [type]<>0 AND TRY_CONVERT(decimal(19,4),[value]) IS NULL)
                    THROW 50400,'Resolve the previous quote before setting a price.',1;
                -- Multiplication of exact scaled integers fits decimal(33,0); round only once at scale four.
                IF EXISTS(SELECT 1 FROM @Lines
                    CROSS APPLY (SELECT CONVERT(decimal(13,0),Quantity*10000) Q,CONVERT(decimal(19,0),CASE WHEN Mode=N'perUnit' THEN Price ELSE NULL END*10000) P) scaled
                    WHERE Mode=N'perUnit' AND Unit IS NOT NULL AND Q IS NOT NULL AND P IS NOT NULL
                      AND Q*P>=CONVERT(decimal(28,0),999999999999999999999995000))
                    THROW 50400,'Line estimate exceeds the supported maximum.',1;
                DECLARE @LegacyFields TABLE(EntryIndex int,[key] nvarchar(4000) COLLATE Latin1_General_100_BIN2,[value] nvarchar(max),[type] int);
                INSERT @LegacyFields SELECT EntryIndex,p.[key],p.[value],p.[type] FROM @EntryFields e CROSS APPLY OPENJSON(CASE WHEN e.[key]=N'legacyPricing' AND e.[type]=5 THEN e.[value] ELSE N'{}' END) p WHERE e.[key]=N'legacyPricing' AND e.[type]=5;
                IF EXISTS(SELECT 1 FROM @EntryFields e WHERE e.[key]=N'legacyPricing' AND e.[type]=5 AND (SELECT COUNT(*) FROM @LegacyFields p WHERE p.EntryIndex=e.EntryIndex)<>6)
                    OR EXISTS(SELECT EntryIndex,[key] FROM @LegacyFields GROUP BY EntryIndex,[key] HAVING COUNT(*)<>1)
                    OR EXISTS(SELECT 1 FROM @LegacyFields WHERE CONVERT(varbinary(max),[key]) NOT IN(CONVERT(varbinary(max),N'quantity'),CONVERT(varbinary(max),N'unitOfMeasure'),CONVERT(varbinary(max),N'unitPrice'),CONVERT(varbinary(max),N'pricingUnit'),CONVERT(varbinary(max),N'pricePerQuantity'),CONVERT(varbinary(max),N'pricingQuantity')) OR [type] NOT IN(0,1))
                    THROW 50400,'Invalid retained quote.',1;
                IF EXISTS(SELECT 1 FROM @LegacyFields WHERE [key] IN(N'quantity',N'pricePerQuantity',N'pricingQuantity',N'unitPrice') AND [type]<>0 AND
                    (([key]=N'unitPrice' AND (@Currency IS NULL OR TRY_CONVERT(decimal(19,4),[value]) IS NULL OR DATALENGTH([value])>40))
                    OR ([key]<>N'unitPrice' AND (TRY_CONVERT(decimal(13,4),[value])<=0 OR TRY_CONVERT(decimal(13,4),[value]) IS NULL OR DATALENGTH([value])>28))
                    OR DATALENGTH([value])<12 OR [value] COLLATE Latin1_General_100_BIN2 LIKE N'%[^0-9.]%'
                    OR CHARINDEX(N'.',[value])<>DATALENGTH([value])/2-4 OR CHARINDEX(N'.',[value],CHARINDEX(N'.',[value])+1)>0
                    OR (LEFT([value],1)=N'0' AND CHARINDEX(N'.',[value])<>2)))
                    OR EXISTS(SELECT 1 FROM @LegacyFields WHERE [key] IN(N'unitOfMeasure',N'pricingUnit') AND [type]<>0 AND CONVERT(varbinary(max),[value]) NOT IN(CONVERT(varbinary(max),N'piece'),CONVERT(varbinary(max),N'carat'),CONVERT(varbinary(max),N'gram'),CONVERT(varbinary(max),N'kilogram'),CONVERT(varbinary(max),N'ounce'),CONVERT(varbinary(max),N'troyOunce'),CONVERT(varbinary(max),N'millimeter'),CONVERT(varbinary(max),N'centimeter'),CONVERT(varbinary(max),N'meter'),CONVERT(varbinary(max),N'parcel'),CONVERT(varbinary(max),N'pair'),CONVERT(varbinary(max),N'set'),CONVERT(varbinary(max),N'pack'),CONVERT(varbinary(max),N'box'),CONVERT(varbinary(max),N'lot')))
                    THROW 50400,'Invalid retained quote.',1;
                DECLARE @LegacyLines TABLE(Quantity decimal(13,4),Unit nvarchar(20),Price decimal(19,4),PricingUnit nvarchar(20),Denominator decimal(13,4),PricingQuantity decimal(13,4),Reference decimal(19,4));
                INSERT @LegacyLines SELECT TRY_CONVERT(decimal(13,4),JSON_VALUE(Legacy,'$.quantity')),JSON_VALUE(Legacy,'$.unitOfMeasure'),TRY_CONVERT(decimal(19,4),JSON_VALUE(Legacy,'$.unitPrice')),JSON_VALUE(Legacy,'$.pricingUnit'),TRY_CONVERT(decimal(13,4),JSON_VALUE(Legacy,'$.pricePerQuantity')),TRY_CONVERT(decimal(13,4),JSON_VALUE(Legacy,'$.pricingQuantity')),Reference FROM @Lines WHERE Legacy IS NOT NULL;
                IF EXISTS(SELECT 1 FROM @LegacyLines WHERE (Price IS NOT NULL AND Reference IS NOT NULL) OR (PricingQuantity IS NOT NULL AND (Unit IS NULL OR PricingUnit IS NULL OR Unit=PricingUnit)))
                    THROW 50400,'Review retained quote basis.',1;
                IF EXISTS(SELECT 1 FROM @LegacyLines
                    CROSS APPLY (SELECT CONVERT(decimal(13,0),(CASE WHEN Unit=PricingUnit THEN Quantity ELSE PricingQuantity END)*10000) Q,
                      CONVERT(decimal(19,0),Price*10000) P,CONVERT(decimal(13,0),Denominator*10000) D) scaled
                    WHERE Quantity IS NOT NULL AND Unit IS NOT NULL AND PricingUnit IS NOT NULL AND Price IS NOT NULL AND Denominator IS NOT NULL AND Q IS NOT NULL
                      AND Q*P*CONVERT(decimal(1,0),2)>=CONVERT(decimal(24,0),199999999999999999999999)*D)
                    THROW 50400,'Retained quote exceeds the supported maximum.',1;
                ...
                BEGIN
                    ...
                    IF @CurrentCurrency IS NOT NULL AND (@Currency IS NULL OR CONVERT(varbinary(max),CONVERT(nvarchar(3),@CurrentCurrency))<>CONVERT(varbinary(max),@Currency))
                        AND (EXISTS(SELECT 1 FROM @EntryFields WHERE [key] IN(N'indicativePrice',N'price') AND [type]<>0) OR EXISTS(SELECT 1 FROM @LegacyFields WHERE [key]=N'unitPrice' AND [type]<>0)
                            OR EXISTS(SELECT 1 FROM Purchasing.DraftOrders d CROSS APPLY OPENJSON(d.ContentJson,'$.entries') e
                                WHERE d.TenantId=@TenantId AND d.Id=@TargetId
                                  AND (JSON_VALUE(e.[value],'$.indicativePrice') IS NOT NULL OR JSON_VALUE(e.[value],'$.unitPrice') IS NOT NULL OR JSON_VALUE(e.[value],'$.price') IS NOT NULL OR JSON_VALUE(e.[value],'$.legacyPricing.unitPrice') IS NOT NULL)))
                    ...
                    UPDATE Purchasing.DraftOrders SET Title=@Title,SupplierName=@Supplier,Currency=@Currency,Notes=@Notes,
                        ...
                        ContentSchemaVersion=3,ContentJson=@Content,UpdatedAtUtc=@Now,UpdatedByUserId=@ActorUserId
                ...
                BEGIN
                    ...
                    INSERT Purchasing.DraftOrders(PoNumber,SupplierId,SupplierContactName,SupplierEmail,SupplierPhone,SupplierWebsite,SupplierPostalAddress,SupplierOrderReference,Platform,Id,TenantId,Title,SupplierName,Currency,Notes,ContentSchemaVersion,ContentJson,CreatedAtUtc,UpdatedAtUtc,CreatedByUserId,UpdatedByUserId)
                        VALUES(@PoNumber,@SupplierId,@ContactName,@Email,@Phone,@Website,@PostalAddress,@SupplierOrderReference,@Platform,@TargetId,@TenantId,@Title,@Supplier,@Currency,@Notes,3,@Content,@Now,@Now,@ActorUserId,@ActorUserId);
                END;
                ...
                INSERT Purchasing.DraftOrderRequestReceipts(TenantId,RequestId,DraftOrderId,Operation,ActorUserId,ExpectedRowVersion,FingerprintVersion,InputFingerprint,ResultRowVersion,CompletedAtUtc)
                    VALUES(@TenantId,@RequestId,@TargetId,@Operation,@ActorUserId,@ExpectedVersion,4,@Fingerprint,@CurrentVersion,@Now);
                COMMIT;
