...
public partial class WorkbenchDbContext
{
    ...
    private void ConfigureDraftOrders(ModelBuilder builder)
    {
        ...
        draft.ToTable("DraftOrders", "Purchasing", table =>
        {
            ...
            table.HasCheckConstraint("CK_DraftOrders_Content", "[ContentSchemaVersion] IN (1,2) AND ISJSON([ContentJson],OBJECT)=1 AND DATALENGTH([ContentJson])<=1048576");
        ...
        receipt.ToTable("DraftOrderRequestReceipts", "Purchasing", table =>
        {
            ...
            table.HasCheckConstraint("CK_DraftOrderRequestReceipts_Operation", "([Operation] COLLATE Latin1_General_100_BIN2='Create' AND [ExpectedRowVersion] IS NULL) OR ([Operation] COLLATE Latin1_General_100_BIN2 IN ('Update','Delete') AND [ExpectedRowVersion] IS NOT NULL)");
            table.HasCheckConstraint("CK_DraftOrderRequestReceipts_Fingerprint", "[FingerprintVersion] IN (1,2,3)");
