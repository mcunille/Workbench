...
public partial class WorkbenchDbContext
{
    ...
    private void ConfigureDraftOrders(ModelBuilder builder)
    {
        ...
        draft.ToTable("DraftOrders", "Purchasing", table =>
        {
            ...
            table.HasCheckConstraint("CK_DraftOrders_DeletedContent", "[IsDeleted]=0 OR ([Title] IS NULL AND [SupplierName] IS NULL AND [Currency] IS NULL AND [Notes] IS NULL AND CONVERT(varbinary(max),[ContentJson])=CONVERT(varbinary(max),N'{\"sourceLinks\":[],\"entries\":[]}'))");
