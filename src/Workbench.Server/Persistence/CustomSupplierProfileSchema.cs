...
// Frozen supplier command installed by MakeSupplierProfilesCustom; earlier migrations retain their original SQL.
internal static class CustomSupplierProfileSchema
{
    internal const string SaveSupplier = """
        ...
        BEGIN
          ...
          IF @RequestId IS NULL OR @RequestId='00000000-0000-0000-0000-000000000000'
            OR @CanonicalInputJson IS NULL OR DATALENGTH(@CanonicalInputJson)>1048576 OR ISJSON(@CanonicalInputJson,OBJECT)<>1
          ...
          BEGIN TRY
            ...
            BEGIN
              ...
              RETURN;
            END;
            DECLARE @Name nvarchar(max),@ContactName nvarchar(max),@Email nvarchar(max),@Phone nvarchar(max),@Website nvarchar(max),@PostalAddress nvarchar(max),@SocialProfiles nvarchar(max);
            ...
            BEGIN
              ...
              IF (SELECT COUNT(*) FROM @Fields WHERE [key] IN(N'name',N'contactName',N'email',N'phone',N'website',N'postalAddress'))<>6 OR EXISTS(SELECT [key] FROM @Fields GROUP BY [key] HAVING COUNT(*)<>1)
                OR EXISTS(SELECT 1 FROM @Fields WHERE ([key]<>N'socialProfiles' AND [type] NOT IN(0,1)) OR ([key]=N'socialProfiles' AND [type] NOT IN(0,4)) OR CONVERT(varbinary(max),[key]) NOT IN(CONVERT(varbinary(max),N'name'),CONVERT(varbinary(max),N'contactName'),CONVERT(varbinary(max),N'email'),CONVERT(varbinary(max),N'phone'),CONVERT(varbinary(max),N'website'),CONVERT(varbinary(max),N'postalAddress'),CONVERT(varbinary(max),N'socialProfiles')))
                THROW 50500,'Review supplier fields.',1;
              ...
              SELECT @SocialProfiles=[value] FROM @Fields WHERE [key]=N'socialProfiles';
              IF @SocialProfiles IS NOT NULL
              BEGIN
                DECLARE @Profiles TABLE([key] nvarchar(4000),[value] nvarchar(max),[type] int);
                INSERT @Profiles SELECT [key],[value],[type] FROM OPENJSON(@SocialProfiles);
                IF (SELECT COUNT(*) FROM @Profiles)>20 OR EXISTS(SELECT 1 FROM @Profiles WHERE [type]<>5)
                  THROW 50500,'Review supplier fields.',1;
                DECLARE @ProfileFields TABLE([entry] nvarchar(4000),[key] nvarchar(4000) COLLATE Latin1_General_100_BIN2,[value] nvarchar(max),[type] int);
                INSERT @ProfileFields SELECT p.[key],f.[key],f.[value],f.[type] FROM @Profiles p CROSS APPLY OPENJSON(p.[value]) f;
                IF EXISTS(SELECT 1 FROM @Profiles p WHERE (SELECT COUNT(*) FROM @ProfileFields f WHERE f.[entry]=p.[key])<>2)
                  OR EXISTS(SELECT [entry],[key] FROM @ProfileFields GROUP BY [entry],[key] HAVING COUNT(*)<>1)
                  OR EXISTS(SELECT 1 FROM @ProfileFields WHERE [type]<>1 OR CONVERT(varbinary(max),[key]) NOT IN(CONVERT(varbinary(max),N'label'),CONVERT(varbinary(max),N'handle'))
                    OR Purchasing.IsSupplierText([value],CASE WHEN [key]=N'label' THEN 100 ELSE 2048 END,0)=0)
                  OR EXISTS(SELECT UPPER([value] COLLATE Latin1_General_100_CI_AS) COLLATE Latin1_General_100_BIN2 FROM @ProfileFields WHERE [key]=N'label' GROUP BY UPPER([value] COLLATE Latin1_General_100_CI_AS) COLLATE Latin1_General_100_BIN2 HAVING COUNT(*)>1)
                  THROW 50500,'Review supplier fields.',1;
                IF NOT EXISTS(SELECT 1 FROM @Profiles) SET @SocialProfiles=NULL;
              END;
            ...
            BEGIN
              ...
              ELSE
                UPDATE Purchasing.Suppliers SET Name=@Name,ContactName=@ContactName,Email=@Email,Phone=@Phone,Website=@Website,SocialProfilesJson=@SocialProfiles,PostalAddress=@PostalAddress,UpdatedAtUtc=@Now,UpdatedByUserId=@ActorUserId WHERE TenantId=@TenantId AND Id=@TargetId;
            END
            ELSE
            BEGIN
              SET @TargetId=NEWID();
              INSERT Purchasing.Suppliers(Id,TenantId,Name,ContactName,Email,Phone,Website,SocialProfilesJson,PostalAddress,IsArchived,CreatedAtUtc,UpdatedAtUtc,CreatedByUserId,UpdatedByUserId)
                VALUES(@TargetId,@TenantId,@Name,@ContactName,@Email,@Phone,@Website,@SocialProfiles,@PostalAddress,0,@Now,@Now,@ActorUserId,@ActorUserId);
            END;
