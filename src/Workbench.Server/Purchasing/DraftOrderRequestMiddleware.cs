...
public sealed class DraftOrderCacheMiddleware(RequestDelegate next)
{
    ...
    {
        if ((context.Request.Path.StartsWithSegments("/api/purchase-order-drafts") || context.Request.Path.StartsWithSegments("/api/v2/purchase-order-drafts") || context.Request.Path.StartsWithSegments("/api/v3/purchase-order-drafts") || context.Request.Path.StartsWithSegments("/api/suppliers")))
