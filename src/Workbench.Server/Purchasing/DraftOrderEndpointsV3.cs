...
public static class DraftOrderEndpointsV3
{
    public static void MapPurchaseOrderDraftsV3(this IEndpointRouteBuilder endpoints)
    {
        var group = endpoints.MapGroup("/api/v3/purchase-order-drafts").WithTags("Purchasing").RequireAuthorization();
        ...
        group.MapGet("/{id:guid}", ReadAsync).Produces<DraftOrderResponseV3>().ProducesProblem(404);
    ...
    private static IResult Calculate(CalculateDraftOrderRequest request)
    {
        var draft = request.Draft is null ? null : DraftOrderInputV3.Normalize(request.Draft);
        var errors = DraftOrderInputV3.Validate(draft);
        return errors.Count > 0 ? Validation(errors) : Results.Ok(DraftOrderInputV3.Calculate(draft!));
    }
    ...
    private static async Task<IResult> ReadAsync(Guid id, WorkbenchDbContext database, CancellationToken cancellationToken)
    {
        ...
        var draft = new DraftContentV3(row.Title, row.SupplierName, row.Currency, row.Notes, links, entries, row.SupplierId, row.SupplierContactName, row.SupplierEmail, row.SupplierPhone, row.SupplierWebsite, row.SupplierPostalAddress, row.SupplierOrderReference, row.Platform);
        return Results.Ok(new DraftOrderResponseV3(row.Id, draft, DraftOrderCursor.Timestamp(row.CreatedAtUtc), DraftOrderCursor.Timestamp(row.UpdatedAtUtc), Convert.ToBase64String(row.RowVersion), PurchasingIdentityInput.Reference(row.PoNumber), archived, DraftOrderInputV3.Calculate(draft)));
    }
    private static Task<IResult> CreateAsync(CreateDraftOrderRequestV3 request, WorkbenchDbContext database,
    ...
    private static Task<IResult> UpdateAsync(Guid id, UpdateDraftOrderRequestV3 request, WorkbenchDbContext database,
        RequestActor actor, HttpContext context, CancellationToken cancellationToken) =>
        SaveAsync(request.RequestId, id, request.ExpectedVersion, request.Draft, database, actor, context, cancellationToken);

    private static async Task<IResult> SaveAsync(Guid requestId, Guid? id, string? version, DraftContentV3? input,
        WorkbenchDbContext database, RequestActor actor, HttpContext context, CancellationToken cancellationToken)
    {
        var draft = input is null ? null : DraftOrderInputV3.Normalize(input);
        var errors = DraftOrderInputV3.Validate(draft);
        ...
        try
        {
            ...
            await using var command = new SqlCommand($"[Purchasing].[{operation}DraftOrderV3]", (SqlConnection)database.Database.GetDbConnection())
            ...
            { Value = DraftOrderInputV3.Canonical(operation, id, expectedVersion, draft!) });
            ...
            var location = $"/api/v3/purchase-order-drafts/{response.DraftOrderId:D}";
