...
public static class DraftOrderEndpoints
{
    ...
    {
        var group = endpoints.MapGroup("/api/beta/purchase-order-drafts").WithTags("Purchasing").RequireAuthorization();
    ...
    {
        var draft = request.Draft is null ? null : DraftOrderInput.Normalize(request.Draft);
        var errors = DraftOrderInput.Validate(draft);
        return errors.Count > 0 ? Validation(errors) : Results.Ok(DraftOrderInput.Calculate(draft!));
    }
        ...
        WorkbenchDbContext database, RequestActor actor, HttpContext context, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var location = $"/api/beta/purchase-order-drafts/{response.DraftOrderId:D}";
