...
// Frozen fingerprint-1 normalization also supplies common text/version rules. Changes must preserve
// historical canonical bytes; retained for the lifetime of stored receipts and schema-1 content.

internal static partial class ReceiptDraftOrderInputV1
{
    ...
    public static ReceiptDraftContentV1 Normalize(ReceiptDraftContentV1 input) => input with
    {
    ...
    public static Dictionary<string, string[]> Validate(ReceiptDraftContentV1? input)
    {
    ...
    public static string ContentJson(ReceiptDraftContentV1 draft) => JsonSerializer.Serialize(new { draft.SourceLinks, draft.Entries }, JsonOptions);

    public static string Canonical(string operation, Guid? targetId, string? expectedVersion, ReceiptDraftContentV1 draft) =>
