...
public sealed record PurchaseOrderSummary(Guid Id, string? Title, string? SupplierName, string UpdatedAtUtc, string PoReference, string? SupplierOrderReference, string? Platform, string State, string? OrderDate, int Revision, string? FirstItemDescription);
