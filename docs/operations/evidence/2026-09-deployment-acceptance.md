# Initial deployment acceptance: 2026-09-06 through 2026-09-09

This closed record retains dates, revisions and coverage limits. Use the
[current acceptance matrix](../production-readiness.md) for reconciled status and the
[evidence index](../deployment-verification.md) for other changes. Statements of remaining work
in earlier sections are historical snapshots, superseded only within later evidence scope.
No hosted checks or prices were refreshed during documentation consolidation.
...
**Historical snapshot before the 2026-09-08 launch.** Read with the later launch and
follow-up sections; this is not the current acceptance checklist.
