...
| `workbench_web` | Tenant-scoped runtime reads/writes; narrow identity resolution, invitation claims, item/photo/acquisition/purchase-draft/supplier commands and readiness procedures. Items allow direct SELECT/INSERT, with UPDATE/DELETE denied; creation snapshots, acquisitions, purchase drafts, suppliers, purchase counters and request receipts are SELECT-only with writes through restricted commands. Purchasing exposes restricted draft create/update/delete and supplier commands; direct table writes remain denied, including reference allocation and retry receipts. | Web credential only in the web workload. No migration history/security-control changes or raw tenant proof-store access. Do not reuse for worker, migration or operator tools. |
