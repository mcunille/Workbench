...
The current required migration is `20260917020000_AddDraftFinancialAdjustments`. Drain incompatible writers before applying it, then deploy the matching API/client together. It updates the V4 commands in place; it adds no V5 API. New old-shape requests are rejected after prior receipt resolution. Verify fresh creation and upgrade from the PO-03 schema with saved drafts and receipts. Down migration is always blocked to preserve financial inputs; use a reviewed forward correction or guarded recovery.
