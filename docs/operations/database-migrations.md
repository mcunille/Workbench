...
The PO-03 migration is `20260917010000_AddSupplierBasedDraftPricing`, following `AddSupplierIdentityAndPurchaseReferences`. It adds content schema 3, V4 fingerprints and restricted supplier-based pricing commands, retaining older content and receipts. V3 commands cannot overwrite schema 3 drafts. New V4 grants are required for readiness and principal provisioning. Verify a fresh database and upgrade from the PO-02 schema, plus legacy reads/conversion and immutable receipt replay. Its down migration is blocked to preserve V4 content; use a reviewed forward correction or guarded recovery.
...
The current required migration is `20260917030000_ProtectConfirmedSupplierChargeCorrections`. Drain incompatible writers before applying it, then deploy the matching API/client together. It updates the V4 commands in place; it adds no V5 API. New old-shape requests are rejected after prior receipt resolution. Verify fresh creation and upgrades from the PO-03 and financial schemas with saved drafts and receipts. Down migration is always blocked to preserve financial inputs; use a reviewed forward correction or guarded recovery.

The supplier-payee guard is a separate forward migration because `AddDraftFinancialAdjustments`
has already been applied to a retained preview. Its migration history and installed predecessor
remain unchanged; do not consolidate by rewriting that applied migration. Existing financial
content and successful request receipts are retained exactly.
