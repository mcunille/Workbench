...
| `20260912030844_AddDraftSupplierOrders` | `AddAcquisitionDocuments` | Tenant-owned draft orders and compact request receipts, RLS, restricted create/update commands, tenant-qualified actor references and checked document replacement. Readiness, provisioning and backup markers advance. Fresh creation and H11 upgrade preserve identity, holdings, acquisitions and documents; draft saves create no inventory or financial records. | Always blocked; retain drafts and retry receipts through forward correction or guarded recovery. |
| `20260912033355_TightenDraftSourceLinkValidation` | `AddDraftSupplierOrders` | Current required schema: rejects malformed hostnames and control characters in draft links through restricted SQL commands. Retains existing drafts and receipts. A forward correction preserves the initial migration already applied to a retained preview. Previous backup manifest versions remain accepted. | Always blocked; use forward correction. |
