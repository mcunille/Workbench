...
| `20260916183834_AddStructuredDraftOrderLines` | `AddSupplierIdentityAndPurchaseReferences` | Previous PO-03 schema: content schema 2 and fingerprint 3, restricted V3 structured-line commands, exact quantity/unit/gross validation and saved-price currency protection. Preserves schema 1 content and V1/V2 receipts; old unmatched writes require reload. Advances readiness, provisioning and backup markers. Verify fresh creation and PO-02 upgrade, legacy reads/replays and deletion. | Always blocked; structured lines and request evidence require forward correction or guarded recovery. |
...
The current required migration is `20260917010000_AddSupplierBasedDraftPricing`, following `AddStructuredDraftOrderLines`. It adds content schema 3, V4 fingerprints and restricted supplier-based pricing commands, retaining older content and receipts. V3 commands cannot overwrite schema 3 drafts. New V4 grants are required for readiness and principal provisioning. Verify a fresh database and upgrade from the structured-line schema, plus legacy reads/conversion and immutable receipt replay. Its down migration is blocked to preserve V4 content; use a reviewed forward correction or guarded recovery.

The two PO-03 migrations remain separate because the structured-line migration was already applied to a retained development preview. It is not rewritten or consolidated after application.