...
| Tenant data path | Tenant claim becomes immutable request context, EF filters/save guards, nonce-bound SQL tenant proof, read-only SQL session context, and RLS filter/block predicates (`src/Workbench.Server/Program.cs:136-148`, `src/Workbench.Server/Tenancy/TenantContextProof.cs:11-80`, `src/Workbench.Server/Persistence/Migrations/20260902070000_HardenSecurityBoundaries.cs:17-113`) |
...
| Recovery operations | Human-run backup/restore, independent restore-pending marker, transactional security sanitation, and readiness checks (`scripts/restore-database.ps1:20-49`, `src/Workbench.Server/Persistence/Migrations/20260902070000_HardenSecurityBoundaries.cs:157-251`) |
...
| 1 | RLS bypass or pooled-connection leakage retains or selects another request's tenant | Web SQL access plus missing/mutable/provable session context or disabled policy | Broad cross-tenant database access | Context values are read-only and nonce-bound to a separate proof key; missing/invalid proof filters all tenant rows; web cannot read the SQL key or alter policy | Keep authorized, credential-only spoof, pooling, permission, and readiness tests; `src/Workbench.Server/Tenancy/TenantContextProof.cs:42-80`, `src/Workbench.Server/Persistence/Migrations/20260902070000_HardenSecurityBoundaries.cs:47-113`, `tests/Workbench.Server.IntegrationTests/TenantIsolationTests.cs:91-107` |
