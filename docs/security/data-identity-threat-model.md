...
| Purchase orders and suppliers | Tenant-filtered reads and restricted SQL commands protect drafts, suppliers, numbering counters and compact retry receipts. Saves check current actor authority, serialize request identity, compare row versions and atomically record a receipt. Supplier links are tenant-qualified; snapshot and platform values belong to the order. New selections serialize with supplier archival. V2 fingerprints cover the new fields; legacy save routes resolve prior receipts without allowing fresh writes. Web table writes and worker access are denied. Draft/supplier input and pending requests remain in browser memory only; authentication loss clears them. `src/Workbench.Server/Purchasing/DraftOrderEndpointsV2.cs`; `src/Workbench.Server/Purchasing/SupplierEndpoints.cs` |
