...
| Purchase order drafts | Tenant-filtered reads and restricted SQL save procedures protect planning content and compact retry receipts. Saves check current actor authority, serialize request identity, compare row versions and atomically record a receipt. Web table writes and worker access are denied. Draft content and pending requests remain in browser memory only; authentication loss clears them. `src/Workbench.Server/Persistence/DraftOrderSchema.cs`; `src/Workbench.Server/Purchasing/DraftOrderEndpoints.cs` |
