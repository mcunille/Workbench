...
| Tenant data path | Tenant claim becomes immutable request context, EF filters/save guards, nonce-bound SQL tenant proof, read-only SQL session context, and RLS filter/block predicates (`src/Workbench.Server/Program.cs:136-148`, `src/Workbench.Server/Tenancy/TenantContextProof.cs:11-80`, `src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:92-157`) |
| Database control plane | Separate setup, web, operator, and migrator roles; exact stored-procedure grants and explicit denials (`src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:543-600`) |
| Recovery operations | Human-run backup/restore, independent restore-pending marker, transactional security sanitation, and readiness checks (`scripts/restore-database.ps1:20-49`, `src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:361-540`) |
...
| Production web | Tenant-context proof key | read-only file selected by `WORKBENCH_TENANT_CONTEXT_PROOF_KEY_FILE` | Separate deployment secret mounted into web replicas; matching value in owner-only SQL table | Web process; database owner provisions SQL copy | nonce-bound SHA-256 proof; web/operator denied table access; readiness verifies denial | `src/Workbench.Server/Security/ProductionSecurityConfigurationValidator.cs:60-78`, `src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:65-74`, `src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:588-599` |
...
- enforce the tenant boundary independently in HTTP authorization, EF, relational constraints, and
  SQL RLS, including pooled connections, identifier substitution, and possession of only the web
  database credential;
...
- limit anonymous login attempts in shared SQL state by normalized-account and trusted-network
  partitions, and keep public recovery/invitation unavailable until real delivery and its complete
  external abuse controls exist (`src/Workbench.Server/Identity/AuthEndpoints.cs:60-90`,
  `src/Workbench.Server/Identity/ISensitiveRequestRateLimiter.cs:46-76`);
...
- Public email delivery is intentionally unavailable in this phase. Login has a shared SQL-backed
  limiter; enabling public recovery/invitations before issue #11 supplies provider integration and
  their complete anti-enumeration/abuse controls is unsupported.
...
| 1 | RLS bypass or pooled-connection leakage retains or selects another request's tenant | Web SQL access plus missing/mutable/provable session context or disabled policy | Broad cross-tenant database access | Context values are read-only and nonce-bound to a separate proof key; missing/invalid proof filters all tenant rows; web cannot read the SQL key or alter policy | Keep authorized, credential-only spoof, pooling, permission, and readiness tests; `src/Workbench.Server/Tenancy/TenantContextProof.cs:42-80`, `src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:92-157`, `tests/Workbench.Server.IntegrationTests/TenantIsolationTests.cs:91-107` |
...
| 1 | Unsafe restore resurrects revoked sessions, reset links, or old key material | Recovery operator restores an older database and resumes traffic without sanitation | Account takeover and rollback of authorization decisions | Restore script writes a non-restored pending marker before multi-user; sanitation clears it transactionally with deletion/version advancement/key invalidation; readiness checks both | Sanitation is mandatory before cutover; `scripts/restore-database.ps1:20-49`, `src/Workbench.Server/Persistence/Migrations/20260904061246_EstablishSecurityBoundaries.cs:361-540` |
...
| 2 | Login/recovery response or timing enumerates accounts | Anonymous requests at scale | User/tenant discovery and targeted credential attacks | Generic login failures, normalized-account plus trusted-network SQL limiter, generic recovery `202`, public delivery disabled | Retain shared login limiter; issue #11 must add real delivery and complete recovery/invitation abuse controls before public enablement; `src/Workbench.Server/Identity/AuthEndpoints.cs:60-90`, `src/Workbench.Server/Identity/RecoveryEndpoints.cs:38-55` |
