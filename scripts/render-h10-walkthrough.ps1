...
$documentationRoot = Join-Path $repositoryRoot 'docs/demos/h10'
...
if (-not $segments -or -not (Test-Path -LiteralPath (Join-Path $mediaRoot 'capture.webm'))) {
    throw 'Run the H10 Playwright walkthrough before rendering.'
}
...
$transcript.Add('# H10 acquisition walkthrough transcript')
...
$arguments += @('-filter_complex', ($filters -join ';'), '-map', '0:v', '-map', '[narration]',
    ...
    '-c:a', 'aac', '-b:a', '128k', '-shortest', '-movflags', '+faststart', 'h10-shared-acquisition.mp4')
Push-Location $mediaRoot
try {
    & $encoder @arguments
    if ($LASTEXITCODE -ne 0) { throw 'Walkthrough render failed.' }
    & $encoder -v error -i h10-shared-acquisition.mp4 -f null -
    if ($LASTEXITCODE -ne 0) { throw 'Walkthrough decode verification failed.' }
} finally { Pop-Location }
Write-Host "Rendered and decoded $(Join-Path $mediaRoot 'h10-shared-acquisition.mp4')"
