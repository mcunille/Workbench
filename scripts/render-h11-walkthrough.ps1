...
$documentationRoot = Join-Path $repositoryRoot 'docs/demos/h11'
...
if (-not $segments -or -not (Test-Path -LiteralPath (Join-Path $mediaRoot 'capture.webm'))) {
    throw 'Run the H11 Playwright walkthrough before rendering.'
}
...
$transcript.Add('# H11 acquisition walkthrough transcript')
...
$arguments += @('-filter_complex', ($filters -join ';'), '-map', '0:v', '-map', '[narration]',
    ...
    '-c:a', 'aac', '-b:a', '128k', '-shortest', '-movflags', '+faststart', 'h11-acquisition-documents.mp4')
Push-Location $mediaRoot
try {
    & $encoder @arguments
    if ($LASTEXITCODE -ne 0) { throw 'Walkthrough render failed.' }
    & $encoder -v error -i h11-acquisition-documents.mp4 -f null -
    if ($LASTEXITCODE -ne 0) { throw 'Walkthrough decode verification failed.' }
} finally { Pop-Location }
Write-Host "Rendered and decoded $(Join-Path $mediaRoot 'h11-acquisition-documents.mp4')"
