...
$mediaRoot = Join-Path $repositoryRoot 'artifacts/h6/walkthrough'
$documentationRoot = Join-Path $repositoryRoot 'docs/demos/h6'
...
if (-not $segments -or -not (Test-Path -LiteralPath (Join-Path $mediaRoot 'capture.webm'))) {
    throw 'Run the H6 Playwright walkthrough before rendering.'
}
Add-Type -AssemblyName System.Speech
$captions = New-Object Collections.Generic.List[string]
$transcript = New-Object Collections.Generic.List[string]
$transcript.Add('# H6 archive recovery walkthrough transcript')
...
try {
    ...
    & $encoder -v error -i h6-archive-recovery.mp4 -f null -
